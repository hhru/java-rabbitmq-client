...
public class DatabaseQueueService {
  ...
  private List<IdentifiedRoutedMessage> getNextBatch(long batchId) {
    ...
    return (List<IdentifiedRoutedMessage>) sessionFactory.getCurrentSession()
      .createNativeQuery("SELECT ev_id, ev_data, ev_type FROM pgq.get_batch_events(:batchId)")
    ...
    }).setParameter("batchId", batchId, LongType.INSTANCE).getResultList();
  }
